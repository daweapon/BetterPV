...
/**
 * Unmodified port - no Minecraft API surface (pure calculator).
 */
	...
	public void getDungeonCompletionWeight(String cataMode) {
		...
		if (cataMode.equals("normal")) {
			...
			for (Map.Entry<String, JsonElement> normalFloor : Utils
				...
				.getAsJsonObject()
				.entrySet()) {
				// Skips non-floor keys such as the "total" entry the current Hypixel API adds.
				JsonElement worth = dungeonsCompletionWorth.get("catacombs_" + normalFloor.getKey());
				...
				double floorScore = amount * worth.getAsDouble();
			...
			for (Map.Entry<String, JsonElement> masterFloor : Utils
				...
				.entrySet()) {
				// Skips non-floor keys such as the "total" entry the current Hypixel API adds.
				JsonElement worth = dungeonsCompletionWorth.get("master_catacombs_" + masterFloor.getKey());
				if (worth == null) continue;
				int amount = masterFloor.getValue().getAsInt();
				double excess = 0;
				if (amount > 1000) {
