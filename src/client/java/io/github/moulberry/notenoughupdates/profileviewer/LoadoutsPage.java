...
public class LoadoutsPage implements GuiProfileViewerPage {
	...
	private static final String HOTF_SKULL =
	...
	private record Tree(String title, String skill, String core, List<Node> nodes, int rows, TreeItems items) {
	}

	private record Node(String id, String type, String name, int x, int y, int max) {
	}
	...
	private static int treeHeight(Tree tree) {
	...
	public void drawPage(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		int miningSlot = current.miningSlot() != null ? current.miningSlot() : selectedSlot(profileInfo, "mining");
		int foragingSlot = current.foragingSlot() != null ? current.foragingSlot() : selectedSlot(profileInfo, "foraging");
		if (miningTree != null) {
	...
	private ItemStack hotmSkull() {
		if (hotmSkull == null) hotmSkull = Utils.createSkull("", "2361bcef6d1c4eb58ca05341c4e80c4c", HOTM_SKULL);
		return hotmSkull;
	}

	private ItemStack hotfSkull() {
		if (hotfSkull == null) hotfSkull = Utils.createSkull("", "3da6d8192916413e88e786247708b93e", HOTF_SKULL);
		return hotfSkull;
	}
	...
	private static int selectedSlot(JsonObject profileInfo, String skill) {
	...
	private void drawTree(
		GuiGraphicsExtractor graphics, Font font, Tree tree, JsonObject profileInfo, int slot, int x, int top, int mouseX, int mouseY
	) {
		...
		for (Node node : tree.nodes()) {
			...
			RenderUtils.drawTexturedRect(graphics, PERK_BACKGROUND, slotX, slotY, 16, 16);
			RenderUtils.drawItemStack(graphics, iconCache.computeIfAbsent("vanilla:" + item, key -> new ItemStack(item)), slotX, slotY);
			...
			if (shown > 1) {
				...
				RenderUtils.text(graphics, font, text, slotX + 17 - font.width(text), slotY + 9, 0xFFFFFF, true);
			}
			if (Utils.isWithinRect(mouseX, mouseY, slotX, slotY, 16, 16)) {
				instance.tooltipToDisplay = nodeTooltip(node, level, disabled, node.id().equals(selectedAbility));
			}
		}
	}
